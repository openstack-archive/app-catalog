...
def process():
    ...
    session['launchpad_name'], session['launchpad_teams'] = get_launchpad_info(
        session['email'])
...
def get_launchpad_info(email):
    r = requests.get(settings.LAUNCHPAD_API_URL + '/people',
                     params={'ws.op': 'getByEmail',
                             'email': email})
    lp_name = r.json()["name"]
    url = settings.LAUNCHPAD_API_URL + "/~%s/super_teams" % lp_name
    r = requests.get(url)
    lp_teams = [e['name'] for e in r.json()["entries"]]
    return lp_name, lp_teams


def get_auth_info(request):
    session = auth.session_interface.open_session(auth, request)
    # TODO: do not show secrets
    return dict(session)
