...
class MCSessionInterface(sessions.SessionInterface):
    ...
    def open_session(self, app, request):
        ...
        if self.sid:
            ...
            if data:
                return Session(**pickle.loads(data))
        return Session()

    def save_session(self, app, session, response):
        if not self.sid:
            self.sid = uuid.uuid4().hex
        self.mc.set(self.sid, pickle.dumps(session),
...
def login():
    ...
    sreg_request = sreg.SRegRequest(required=['fullname', 'email'])
...
def info():
    ...
    return Response(json.dumps(auth_info), status=200, mimetype='application/json')


def get_launchpad_name(email):
    r = requests.get(settings.LAUNCHPAD_API_URL + '/people',
                     params={'ws.op': 'getByEmail',
                             'email': email})
    return r.json()["name"]

def is_admin(email):
    launchpad_name = get_launchpad_name(email)
    url = settings.LAUNCHPAD_API_URL + "/~%s/super_teams" % launchpad_name
    r = requests.get(url)
    for team in r.json()["entries"]:
        if team["name"] in settings.LAUNCHPAD_ADMIN_GROUPS:
            return True
