(function() {
  ...
  var orderedFields = ["name", "description", "license", "license_url", "icon", "metadata", "tags"];
  angular
    .module("AppCatalog")
  ...
  function EditAssetController($http, $routeParams, $location, UrlService) {
    ...
    function setFormFields(schemas) {
      ...
      for (var i=0; i<orderedFields.length; i++) {
        var field = getFieldWidget(i.name, schemas[vm.type].properties[orderedFields[i]]);
        field.name = orderedFields[i];
    ...
    function isBlob(item) {
      return 'external' in item && 'checksum' in item && 'content_type' in item;
    }
