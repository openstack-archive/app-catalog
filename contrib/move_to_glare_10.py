...
def _extract_keys(src, dst, *keys):
    for key in keys:
        if key in src:
            dst[key] = src[key]
...
class AssetUploader(object):
    ...
    def upload_assets(self):
        for name in self._assets:
            try:
                ...
                logging.error("Failed to create asset %s" % name)
    ...
    def _get_common_data(self, asset):
        data = {}
        _extract_keys(asset, data, "name", "description", "depends",
                      "provided_by", "release", "icon", "license",
                      "license_url", "supported_by")
        return data

    def _process_glance(self, asset):
        data = self._get_common_data(asset)
        _extract_keys(asset, data, "min_disk", "min_ram", "disk_format",
    ...
    def _process_heat(self, asset):
        data = self._get_common_data(asset)
        _extract_keys(asset, data, "environment", "template_version")
        template = self._create_asset("heat_template", data)
        self._create_blob("heat_template", template["id"], "template",
                          asset["attributes"]["url"])
        self._activate_asset("heat_template", template["id"])
